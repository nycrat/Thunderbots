...
class GenericAngle
{
   public:
     ...
     */
    constexpr Derived operator*(double scale) const;
     ...
     */
    constexpr Derived operator/(double divisor) const;
...
inline bool GenericAngle<T>::operator==(const T& y) const
{
    ...
    // T diff = x.clamp().minDiff(y.clamp());
    // return diff.rads <= FIXED_EPSILON;
}

template <typename T>
inline constexpr bool GenericAngle<T>::operator!=(const T& y) const
{
    return !(T::fromRadians(rads) == y);
}
